...
public class MonteCarloTest {
    public static void piDemo() {
        ...
        BiFunction<Long, Supplier<Integer>, Double> monteCarlo = (n, experiment) ->
                Mathx.sum(
                        limit(map(generate(experiment), binary -> binary), n)) / n;
        ...
        Function<Supplier<Integer>, Iterator<Double>> monteCarloIterator =
                supplier -> Iterators.zip(monteCarlo,
                        Iterators.iterate(1L, x -> x + 1),
                        Iterators.iterate(supplier, x -> x)
                );
//
//        /*
...
//
        ...
        System.out.println(Iterators.get(pi, 100_1000));
    }
    ...
    private static void potionTestWithInfiniteIterators() {
        ...
        Iterator<Quality> herbQualities = Iterators.zip(
                (availability, quality) ->
                        Objects.equals(availability, 1) ? Quality.BEST : Quality.values()[quality], herbAvailablities,
                qualities);
         ...
         */
        List<Double> herbList = Iterators.toList(limit(medicineEffects, 100));
        herbList.iterator().forEachRemaining(System.out::println);
    }
    ...
    private static void potionTestWithExperiments() {
        ...
        Experiments<Integer> herbQualities = new Experiments<>(
                Iterators.zip((available, effect) -> available == 1 ? Quality.BEST.ordinal() : effect,
        ...
        Iterators.toList(limit(medicineEffects, 7000)); // 실제 계산은 여기서 일어납니다!!!
